...
export async function registerLiveGameServiceWorker(): Promise<ServiceWorkerRegistration | null> {
  ...
  return navigator.serviceWorker.register('/live-game-sw.js', { scope: '/games/' })
}

export function subscribeToLiveGameSwMessages(handler: (message: LiveGameSwMessage) => void): () => void {
  ...
  const onMessage = (event: MessageEvent<unknown>) => {
    ...
    handler({
      type: 'GAME_UPDATE',
      data: typeof message.data === 'object' && message.data != null
...
export async function subscribeToLiveGamePush(gameId: string): Promise<boolean> {
...
export async function unsubscribeFromLiveGamePush(gameId: string): Promise<void> {
  if (typeof window === 'undefined') return
  if (!('serviceWorker' in navigator)) return
...
export function createScreenWakeLockManager(onStateChange?: (isActive: boolean) => void): WakeLockManager {
  ...
  const request = async (): Promise<boolean> => {
    ...
    try {
      ...
      sentinel.addEventListener('release', () => {
        sentinel = null
        notify(false)
        if (document.visibilityState === 'visible' && !isDestroyed && autoAcquire) {
          void request()
        }
      })
      return true
  ...
  const releaseInternal = async (preserveAutoAcquire: boolean): Promise<void> => {
