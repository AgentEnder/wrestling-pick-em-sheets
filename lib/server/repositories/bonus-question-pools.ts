...
import type { BonusQuestionPools, BonusQuestionTemplates } from '@/lib/server/db/generated'
import { requireDbId } from '@/lib/server/db/types'
import type { BonusPoolRuleSet, BonusQuestionPool, BonusQuestionTemplate, BonusQuestionValueType } from '@/lib/types'
...
function normalizeRuleSetIds(value: readonly string[] | undefined): BonusPoolRuleSet[] {
  if (!Array.isArray(value)) return []
...
function normalizeValueType(value: unknown, isTimeBased: number | boolean, isCountBased: number | boolean): BonusQuestionValueType {
  if (value === 'numerical' || value === 'time' || value === 'rosterMember') return value
  if (Number(isTimeBased) === 1) return 'time'
  if (Number(isCountBased) === 1) return 'numerical'
  return 'string'
}

function mapTemplate(row: BonusQuestionTemplateSelectable): BonusQuestionTemplate {
  const answerType = row.answer_type === 'multiple-choice' ? 'multiple-choice' : 'write-in'
  ...
  return {
    ...
    answerType,
    options: answerType === 'multiple-choice' ? parseJsonArray(row.options_json) : [],
    valueType: normalizeValueType(row.value_type, row.is_time_based, row.is_count_based),
...
export async function listBonusQuestionPools(input?: {
  ...
  const poolIds = poolRows.map((row) => requireDbId(row.id, 'bonus_question_pools.id'))
...
export async function createBonusQuestionPool(input: {
  ...
  await db
    .insertInto('bonus_question_pools')
    .values(values)
    .execute()
...
export async function updateBonusQuestionPool(
  ...
  if (input.matchTypeIds !== undefined) {
    update.match_type_ids_json = JSON.stringify(normalizeMatchTypeIds(input.matchTypeIds))
  }
  if (input.ruleSetIds !== undefined) {
    update.rule_set_ids_json = JSON.stringify(normalizeRuleSetIds(input.ruleSetIds))
  }
...
export async function deleteBonusQuestionPool(poolId: string): Promise<boolean> {
  const result = await db
...
export async function createBonusQuestionTemplate(input: {
  ...
  await db
    .insertInto('bonus_question_templates')
    .values(values)
    .execute()
...
export async function updateBonusQuestionTemplate(
  ...
  if (input.questionTemplate !== undefined) update.question_template = input.questionTemplate
  if (input.defaultPoints !== undefined) update.default_points = input.defaultPoints
  ...
  if (input.options !== undefined) update.options_json = JSON.stringify(input.options)
  ...
  if (input.defaultSection !== undefined) update.default_section = input.defaultSection
  if (input.sortOrder !== undefined) update.sort_order = input.sortOrder
  if (input.isActive !== undefined) update.is_active = input.isActive ? 1 : 0
...
export async function deleteBonusQuestionTemplate(templateId: string): Promise<boolean> {
  const result = await db
