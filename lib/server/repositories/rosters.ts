...
function matchesPromotionQuery(
  promotion: Promotion,
  normalizedPromotionName: string,
): boolean {
...
export async function getRosterSuggestionsByPromotionName(input: {
  ...
  const members = await listPromotionRosterMembers(matchedPromotion.id, {
  ...
  for (const member of members) {
    ...
    const matchesQuery =
      ...
      candidateNames.some((candidate) =>
        normalizeName(candidate).includes(normalizedQuery),
      );
