...
function mapMatchType(row: MatchTypeSelectable): MatchType {
  const defaultRuleSetIds = normalizeRuleSetIds(parseJsonArray(row.default_rule_set_ids_json))
...
export async function updateMatchType(
  ...
  if (input.defaultRuleSetIds !== undefined) {
    ...
    update.default_is_battle_royal = defaultRuleSetIds.includes('timed-entry') ? 1 : 0
  }
