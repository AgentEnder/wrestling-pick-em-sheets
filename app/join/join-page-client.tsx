...
import { getLiveGameJoinPreview, joinLiveGame, type LiveGameJoinPreviewResponse } from '@/lib/client/live-games-api'
import { SignInButton, SignedIn, SignedOut, UserButton, useUser } from '@/lib/client/clerk-test-mode'
...
function buildClerkNicknameCandidate(rawUser: unknown): string {
  ...
  const primaryEmail = user.primaryEmailAddress as { emailAddress?: string } | null | undefined
  const email = typeof primaryEmail?.emailAddress === 'string' ? primaryEmail.emailAddress.trim() : ''
  if (email.includes('@')) {
...
export default function JoinPageClient() {
  ...
  const clerkNickname = useMemo(() => buildClerkNicknameCandidate(user), [user])
  ...
  const [joinPreview, setJoinPreview] = useState<LiveGameJoinPreviewResponse | null>(null)
  const [isLoadingPreview, setIsLoadingPreview] = useState(false)
  const [nowTickMs, setNowTickMs] = useState(Date.now())

  useEffect(() => {
    if (typeof window === 'undefined') return
    const storedNickname = window.localStorage.getItem(LAST_NICKNAME_STORAGE_KEY)?.trim() ?? ''
    if (storedNickname) {
  ...
  async function handleJoin(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const joined = await joinLiveGame(joinCode, normalizedNickname, deviceInfo)
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-md items-center px-4 py-10">
      <form onSubmit={handleJoin} className="w-full rounded-xl border border-border bg-card p-5">
        ...
        <div className="mt-3 rounded-md border border-border/70 bg-background/40 px-3 py-2">
          <SignedIn>
            <p className="text-sm">
              Joining as <span className="font-medium">{accountLabel ?? 'Signed-in user'}</span>
            </p>
            <p className="mt-0.5 text-xs text-muted-foreground">Your account is linked to this room entry.</p>
          </SignedIn>
          <SignedOut>
            <p className="text-xs text-muted-foreground">Joining as guest. Sign in to link this entry to your account.</p>
        ...
        <div className="mt-4 rounded-md border border-border/70 bg-background/40 px-3 py-2">
          {isLoadingPreview ? (
            <p className="text-xs text-muted-foreground">Checking room status...</p>
          ) : joinPreview ? (
            <div className="space-y-1">
              <p className="text-sm font-medium">{joinPreview.eventName}</p>
              <p className="text-xs text-muted-foreground">
                Status: <span className="capitalize text-foreground">{joinPreview.status}</span>
              </p>
              {joinPreview.isStarted ? (
                ...
                <p className="text-xs text-muted-foreground">Start time not set</p>
              )}
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">Enter a valid join code to preview start time.</p>
          )}
        </div>
        ...
        {!isSignedIn ? (
          <p className="mt-2 text-center text-xs text-muted-foreground">Guest joins are supported.</p>
        ) : null}
      </form>
    </div>
  )
