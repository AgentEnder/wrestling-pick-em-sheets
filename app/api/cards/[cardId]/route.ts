...
const legacyStandardMatchSchema = z
  .object({
    id: z.string().uuid(),
...
const legacyBattleRoyalMatchSchema = z
  .object({
    id: z.string().uuid(),
...
const saveCardSchema = z.object({
  ...
  matches: z
    .array(
      z.union([
        legacyStandardMatchSchema,
        legacyBattleRoyalMatchSchema,
        normalizedMatchSchema,
      ]),
    )
    .max(MAX_MATCHES),
  eventBonusQuestions: z
    .array(bonusQuestionSchema)
    .max(MAX_BONUS_QUESTIONS)
    .optional()
    .default([]),
});
