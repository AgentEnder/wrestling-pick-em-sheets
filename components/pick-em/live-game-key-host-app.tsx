...
export function LiveGameKeyHostApp({
  ...
  const [activeJoinReviewPlayerId, setActiveJoinReviewPlayerId] = useState<
    string | null
  >(null);
    ...
    setPayload((prev) => {
      ...
      const nextResult: LiveKeyMatchResult = {
        ...
        battleRoyalEntryOrder:
          index === -1 ? [] : nextResults[index].battleRoyalEntryOrder,
  ...
  function removeBattleRoyalEntrant(matchId: string, entrantIndex: number) {
    setPayload((prev) => {
      ...
      nextResults[index] = {
        ...nextResults[index],
        battleRoyalEntryOrder: nextResults[index].battleRoyalEntryOrder.filter(
          (_, i) => i !== entrantIndex,
        ),
      };
            ...
            ...prev.scoreOverrides.filter(
              (o) =>
                !(
                  ...
                  normalizeText(o.playerNickname) === normalizedNickname
                ),
            ),
            {
  ...
  function updateTimer(
    timerId: string,
    updater: (timer: LiveKeyTimer) => LiveKeyTimer,
  ) {
    setPayload((prev) => {
      if (!prev) return prev;
      return {
  ...
  const syncPayload = useCallback(
    async (mode: "manual" | "auto") => {
      ...
      try {
        ...
        if (
          payloadRef.current &&
          snapshotPayload(payloadRef.current) === payloadSnapshot
        ) {
          setIsDirty(false);
        }
  ...
  useEffect(() => {
    ...
    for (const match of card?.matches ?? []) {
      const matchResult = payload.matchResults.find(
    ...
    for (const question of card?.eventBonusQuestions ?? []) {
      if (
        ...
        continue;
      const keyAnswer =
        payload.eventBonusAnswers.find((a) => a.questionId === question.id)
          ?.answer ?? "";
      if (!keyAnswer.trim()) continue;
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-4 px-4 py-6">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:justify-end">
            ...
            <Button
              ...
              disabled={
                game.status === "ended" || isEndingGame || isStatusSaving
              }
      ...
      {card.matches.map((match, index) => {
        ...
        const timerElapsed = timer
          ? formatDuration(getTimerElapsedMs(timer, clockMs))
          : "--:--";
          ...
          >
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
              <div className="space-y-2">
                <Label>Winner</Label>
                <Select
                  value={matchResult?.winnerName || "__none__"}
                  onValueChange={(value) =>
                    ...
                    >
                      <div className="mb-1 flex items-center justify-between gap-2">
                        ...
                        <Button
                          ...
                          onClick={() =>
                            toggleMatchBonusLock(match.id, question.id)
                          }
                          disabled={isLockSaving}
                        >
