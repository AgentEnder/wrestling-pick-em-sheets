  ...
  return {
    ...
    matchResults: payload.matchResults.map((result) => ({
      ...
      bonusAnswers: result.bonusAnswers.map((answer) =>
        answer.timerId === timerId ? { ...answer, timerId: null } : answer,
      ),
    })),
    ...
    tiebreakerTimerId:
      payload.tiebreakerTimerId === timerId ? null : payload.tiebreakerTimerId,
  };
}
...
export function LiveKeyApp({ cardId }: LiveKeyAppProps) {
  ...
  const customTimers = useMemo(
    () =>
      payload.timers.filter((timer) =>
        timer.id.startsWith(CUSTOM_TIMER_PREFIX),
      ),
    [payload.timers],
  );
  ...
  const updateTimer = useCallback(
    (timerId: string, updater: (timer: LiveKeyTimer) => LiveKeyTimer) => {
      setPayload((prev) => {
        const timerIndex = prev.timers.findIndex(
  ...
  function addCustomTimer() {
    setPayload((prev) => ({
      ...
      timers: [
        ...prev.timers,
        createCustomTimer(`Custom Timer ${customTimers.length + 1}`),
      ],
    }));
  }
  ...
  const setBattleRoyalEntryOrder = useCallback(
    (matchId: string, entryOrder: string[]) => {
      setPayload((prev) => {
        const nextResults = [...prev.matchResults];
        const existingIndex = nextResults.findIndex(
          (result) => result.matchId === matchId,
  ...
  const addBattleRoyalEntrant = useCallback(
    (matchId: string, entrantName: string) => {
      const entrant = entrantName.trim();
      if (!entrant) return;

      const existingEntries =
        findMatchResult(payload, matchId)?.battleRoyalEntryOrder ?? [];
  ...
  const removeBattleRoyalEntrant = useCallback(
    (matchId: string, entryIndex: number) => {
      const existingEntries =
        findMatchResult(payload, matchId)?.battleRoyalEntryOrder ?? [];
  ...
  const setMatchBonusAnswer = useCallback(
    (
      matchId: string,
      questionId: string,
      answer: string,
      ...
      setPayload((prev) => {
        const nextResults = [...prev.matchResults];
        let existingIndex = nextResults.findIndex(
          (result) => result.matchId === matchId,
        );
        ...
        const timerId = isTimeBased
          ? (existingAnswer?.timerId ??
            toMatchBonusTimerId(matchId, questionId))
          : null;
  ...
  const setMatchBonusTimer = useCallback(
    (matchId: string, questionId: string, timerId: string | null) => {
      setPayload((prev) => {
        const nextResults = [...prev.matchResults];
  ...
  const setEventBonusAnswer = useCallback(
    (questionId: string, answer: string, isTimeBased: boolean) => {
      setPayload((prev) => {
  ...
  const incrementMatchBonusCount = useCallback(
    (matchId: string, questionId: string, isTimeBased: boolean) => {
  ...
  function setTiebreakerAnswer(answer: string) {
    setPayload((prev) => ({
      ...
      tiebreakerRecordedAt:
        card?.tiebreakerIsTimeBased && answer.trim() ? nowIso() : null,
    }));
  }
  ...
  function applySpecificTimerValueToMatchBonus(
    matchId: string,
    questionId: string,
    timerId: string,
  ) {
  ...
  function applySpecificTimerValueToEventBonus(
    questionId: string,
    timerId: string,
  ) {
    const timer = timersById.get(timerId);
  ...
  const hasEventBonusQuestions = useMemo(
    () => (card?.eventBonusQuestions?.length ?? 0) > 0,
    [card],
  );
                    ...
                    <div className="grid gap-2 md:grid-cols-[1fr_auto] md:items-center">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={timer.label}
                          onChange={(event) =>
                            setTimerLabel(timer.id, event.target.value)
                          }
                        />
                      </div>
                    ...
                    {winnerSelectValue === "__custom__" ? (
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          value={winnerName}
                          onChange={(event) =>
                            setMatchWinner(match.id, event.target.value)
                          }
                        />
                      </div>
                    ) : null}
              ...
              {match.bonusQuestions.length > 0 ? (
                <div className="mt-4 space-y-3">
                  ...
                  {match.bonusQuestions.map((question) => {
                    ...
                    const isNumericalValueType =
                      questionValueType === "numerical";
                    ...
                    const bonusTimerId = toMatchBonusTimerId(
                      match.id,
                      question.id,
                    );
                    ...
                    const selectedTimerId =
                      answer?.timerId ??
                      (isTimeValueType ? bonusTimerId : null);
                    const bonusTimerElapsed = bonusTimer
                      ? formatDuration(
                          getTimerElapsedMs(bonusTimer, currentTimeMs),
                        )
                      : "--:--";
                                ...
                                <div className="rounded-md border border-border/60 bg-background/40 p-2.5">
                                  ...
                                  <div className="mt-1 flex flex-wrap items-center gap-2">
                                    ...
                                    <Button
                                      ...
                                      variant="outline"
                                      onClick={() =>
                                        bonusTimer && resetTimer(bonusTimer.id)
                                      }
                                      disabled={!bonusTimer}
                                    >
                                      <RotateCcw className="h-4 w-4" />
                ...
                <div className="space-y-2">
                  <div className="grid gap-2 md:grid-cols-[1fr_auto]">
                    <Select
                      ...
                      onValueChange={(value) =>
                        setTiebreakerTimerId(value === "none" ? null : value)
                      }
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue placeholder="Select timer" />
