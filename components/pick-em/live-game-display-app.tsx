...
import { Globe, Laptop, Monitor, RefreshCcw, Smartphone, Sparkles, Trophy, UserRound } from 'lucide-react'
...
function hasDisplayStateChanged(previous: LiveGameStateResponse, next: LiveGameStateResponse): boolean {
...
export function LiveGameDisplayApp({ gameId, joinCodeFromUrl }: LiveGameDisplayAppProps) {
  ...
  const [joinQrCodeDataUrl, setJoinQrCodeDataUrl] = useState<string | null>(null)
        ...
        if (previous) {
          ...
          if (hasLeaderboardChanged(previous, loaded)) {
            const bubbleSteps = buildBubbleSortSteps(
              previous.leaderboard.slice(0, FULLSCREEN_LEADERBOARD_LIMIT).map((entry) => entry.nickname),
              loaded.leaderboard.slice(0, FULLSCREEN_LEADERBOARD_LIMIT).map((entry) => entry.nickname),
            )
            queuedFullscreenEffects.push({
              kind: 'leaderboard',
  ...
  useEffect(() => {
    ...
    async function generateJoinCodeQr() {
      ...
      if (isLoopbackHost(window.location.hostname)) {
        try {
          const response = await fetch('/api/runtime/lan-origin', { cache: 'no-store' })
          if (response.ok) {
      ...
      const joinUrl = `${joinBaseOrigin}/join?code=${encodeURIComponent(currentState.game.joinCode)}`
      if (!cancelled) {
            ...
            <div className="lg-fullscreen-effect-panel">
              ...
              <div className="lg-fullscreen-effect-body">
                {activeFullscreenEffect.events.map((event, index) => (
                  ...
                  >
                    <p className="text-xs uppercase tracking-wide text-primary/90">{formatEventTypeLabel(event.type)}</p>
      ...
      {state.game.status === 'lobby' ? (
        <div className="grid gap-5 lg:grid-cols-[1.1fr_1.4fr] lg:items-start">
          <section className="rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xl shadow-black/25 backdrop-blur">
            <h2 className="text-sm uppercase tracking-wide text-muted-foreground">Scan To Join</h2>
            <div className="mt-3 flex justify-center rounded-xl border border-border/70 bg-background/45 p-5">
              {joinQrCodeDataUrl ? (
                ...
                <p className="py-24 text-sm text-muted-foreground">Waiting for LAN/public URL...</p>
              )}
            </div>
            <p className="mt-3 text-center text-xs text-muted-foreground">
              ...
              and enter <span className="font-mono text-foreground">{state.game.joinCode}</span>
            </p>
          </section>

          <section className="rounded-2xl border border-border/70 bg-card/90 p-4 shadow-xl shadow-black/25 backdrop-blur">
            <h2 className="mb-2 text-sm uppercase tracking-wide text-muted-foreground">Joined Players</h2>
            <div className="space-y-2">
              {state.joinedPlayers.length > 0 ? (
                state.joinedPlayers.map((player) => {
                  ...
                  const DeviceIcon = player.deviceType && player.deviceType.toLowerCase().includes('mobile')
                    ? Smartphone
                    : player.deviceType && player.deviceType.toLowerCase().includes('tablet')
                      ? Smartphone
                      : player.deviceType && player.deviceType.toLowerCase().includes('desktop')
                        ? Monitor
                        : Laptop
                      ...
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-muted-foreground">
                            ...
                            {player.authMethod === 'clerk' ? ' • Account' : ' • Guest'}
                          </p>
                        </div>
            ...
            <div className="space-y-2">
              {state.events.length > 0 ? (
                state.events.slice(0, 15).map((event) => (
                  <div key={event.id} className="rounded-md border border-border/70 bg-background/45 p-2">
