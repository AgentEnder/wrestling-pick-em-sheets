  ...
  const lastEditedCard = useMemo(
    () =>
      cards
        ...
        .sort((a, b) => toTimestamp(b.updatedAt) - toTimestamp(a.updatedAt))[0] ?? null,
    [cards],
  )
    ...
    <div className="relative min-h-screen overflow-x-clip bg-background">
      ...
      <main className="relative z-10 mx-auto grid w-full max-w-6xl gap-5 px-4 py-6 lg:grid-cols-[1.4fr_1fr]">
        <section className="rounded-2xl border border-border/70 bg-card/75 p-5 shadow-[0_20px_40px_rgba(0,0,0,0.25)] backdrop-blur">
          <p className="text-xs uppercase tracking-[0.18em] text-primary">Live Rooms</p>
          <h2 className="mt-2 text-3xl font-semibold leading-tight">Join A Running Game</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Enter a room code or scan the TV QR code to jump into picks immediately.
          </p>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <p className="mt-1 text-sm text-muted-foreground">Use a join code on your phone.</p>
            </Link>
            <Link
              href="/cards"
            ...
            >
              ...
              <p className="mt-1 text-sm text-muted-foreground">Create cards, templates, and host rooms.</p>
            </Link>
          </div>
        </section>
          ...
          <div className="rounded-2xl border border-border/70 bg-card/75 p-4 shadow-[0_20px_40px_rgba(0,0,0,0.25)] backdrop-blur">
            ...
            <SignedIn>
              <p className="mt-2 text-sm">
                Signed in as <span className="font-medium">{accountLabel ?? "Unknown user"}</span>
              </p>
            </SignedIn>
            <SignedOut>
              <p className="mt-2 text-sm text-muted-foreground">You can join as guest, or sign in for account-linked joins.</p>
              <SignInButton mode="modal">
                <Button variant="outline" size="sm" className="mt-3">Sign in to continue</Button>
              </SignInButton>
            </SignedOut>
          </div>

          <div className="rounded-2xl border border-border/70 bg-card/75 p-4 shadow-[0_20px_40px_rgba(0,0,0,0.25)] backdrop-blur">
            <p className="text-xs uppercase tracking-wide text-muted-foreground">Last Edited Card</p>
            {isLoading ? (
              <p className="mt-2 text-sm text-muted-foreground">Loading cards...</p>
            ) : lastEditedCard ? (
              <div className="mt-2">
                <p className="font-medium">{lastEditedCard.name}</p>
                <p className="mt-1 text-xs text-muted-foreground">Updated {formatDate(lastEditedCard.updatedAt)}</p>
